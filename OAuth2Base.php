...
abstract class OAuth2Base {
  ...
  private function getAccessTokenParams() {
    ...
    if ($auth_header !== FALSE) {
      ...
      if (strcmp(substr($auth_header, 0, 7), "Bearer ") !== 0)
        $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Auth header found that doesn\'t start with "Bearer"');
  ...
  public function finishClientAuthorization($is_authorized, $params = array()) {
    ...
    if ($is_authorized === FALSE || $redirect_uri !== $this->getRedirectUri($client_id)) {
    ...
    if (function_exists("apache_request_headers")) {
      $headers = apache_request_headers();
		...
		if (array_key_exists("authorization", array_change_key_case($headers)))
			return array_change_key_case($headers)["authorization"];
